...
page_init(void)
{
	...
	for (i = 0; i < npages; i++) {
		...
		// AP bootstrap code page at MPENTRY_PADDR
		if (pa == MPENTRY_PADDR) {
			pages[i].pp_ref = 1;
			pages[i].pp_link = NULL;
			continue;
...
{
	// Fill this function in
	pte_t *pte = pgdir_walk(pgdir, va, 1); //returns a pointer to PTE for linear address 'va'
    if (pte == NULL) { // if page table couldn't be allocated
        return -E_NO_MEM;
    }

	//pp->pp_ref should be incremented 
	// increment the reference count of the new page before you remove the old one for corner case?
	pp->pp_ref++;
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	//
	// Your code here:
	size_t offset = PGOFF(pa);
	size_t mapped_size = ROUNDUP(size + offset, PGSIZE);
	uintptr_t map_base = base;
	uintptr_t next_base = base + mapped_size;

	if (next_base < base || next_base > MMIOLIM)
		panic("mmio_map_region: overflow");

	boot_map_region(kern_pgdir, map_base, mapped_size, ROUNDDOWN(pa, PGSIZE),
	                PTE_W | PTE_PCD | PTE_PWT);
	base = next_base;

	return (void *) (map_base + offset);
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (a = ROUNDDOWN(start, PGSIZE); a < end; a += PGSIZE) {
		pte = pgdir_walk(env->env_pgdir, (void *) a, 0);
		if (a >= ULIM || pte == NULL || ((*pte & req) != req)) {
			user_mem_check_addr = (a < start) ? start : a;
			return -E_FAULT;
		}
	}
	return 0;
